	...
	if( /^[0-9]/ ) {
		...
		if( current_year != year ) {
			if( current_year != "" ) {
				...
				printf( "%s SP:%.3f\tYGain:%g\tYwithdraw:%.3f\n", current_year, sap, gain, max_withdraw);
			...
			ret = $2;
			if( max_return < ret) max_return = ret;
			wd = 1 - ret / max_return;
			if( max_withdraw < wd) max_withdraw = wd;
			if( t_max_return < ret) t_max_return = ret;
			wd = 1 - ret / t_max_return;
			if( t_max_withdraw < wd) t_max_withdraw = wd;
...
END{
	...
	printf( "sum  SP:%.3f\tYGain:%g\tYwithdraw:%.3f\n", sap, gain, t_max_withdraw);
