#!/bin/sh
#
awk '{ if( $3 >= -0.002 && $3 <= 1.999) print $1,$2,$4,$3;}'|
awk 'BEGIN{
	...
	busy_gain_signal = 0.0135;

	trade_size = 0;
	trade_seq[0] = 0;

	trade_select["123456"] = 0;
	delete trade_select["123456"];
...
}
{
	if( current_date != $1 ) {
		if( current_date != "" ) {
			...
			printf( "%s\t%d\t%.4g\t%.4g\tDAYWIN[%g%%]\tCURRATE[%g%%]\tAVGWIN[%g%%]\t[[%s:%d]]\n",
					current_date, trade_num, sum_gain, cur_d_gain, trade_win_day_rate * 100,
					trade_cur_win_rate * 100, trade_avg_win_rate * 100,( wait > 0 ) ? "WAIT":"BUSY", ( wait > 0 ) ? trade_size : try_trade );


			for( stk in trade_select ){
				if( wait <= 0 ) {
					#printf("%s %s %g\n", current_date, stk, trade_select[stk] );
				}
				delete trade_select[stk];
			}
