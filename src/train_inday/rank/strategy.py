...
def generate_trade_df():
    ...
    names = ['stock']
    dtype = {'stock': np.str_}
    gz2000_df = pd.read_csv('dzh', header=None, sep='\t', names=names, index_col=False, dtype=dtype)
    ...
    for k, g_df in df.groupby(['stock']):
        ...
        g_df['v30'] = g_df['return'].shift(1).rolling(30, min_periods=1).std()
        g_df['v5'] = g_df['return'].shift(1).rolling(5, min_periods=1).std()
        g_df['flag'] = np.where(g_df['return'] > 0, 1, 0)
        g_df['d'] = g_df['flag'].shift(1).rolling(10, min_periods=1).sum()
        g_df['open_next1'] = g_df['open'].shift(-1)
        g_df['low_next1'] = g_df['low'].shift(-1)
        g_df['open_next2'] = g_df['open'].shift(-2)
        #g_df['return2'] = (g_df['open_next2'] - g_df['open_next1']) / g_df['open_next1']
        #g_df['return2'] = np.where(g_df['close'] >= g_df['low_next1'], (g_df['open_next2'] - g_df['close']) / g_df['close'], (g_df['open_next2'] - g_df['low_next1']) / g_df['low_next1'])
        res.append(g_df)
    df = pd.concat(res, axis=0)
    #df = df.loc[(df.v > 0.03) & (df.v < 0.04)]
    df = df.loc[(df.v > 0.04)]
    df = df.loc[(df.v5 > df.v30 )]
    #df = df.loc[df.d <= 3 ]
    ...
    df = df.loc[ ((df.limit_up / df.close - 1) > 0.01) & (df.volume * df.avg * 100 > 20000000)] 
