...
def generate_trade_df():
    ...
    daily_df['open_date'] = daily_df['date'].shift(9)
    daily_df['open_close'] = daily_df['close'].shift(9)
    ...
    trade_fn = 'sta'
    names = ['date', 'stock', 'name', 'buy_price', 'sell_price', 'acc', 'id', 'sell_order_price', 'sell_avg_price', 'sell_time', 'state', 'amt']
    dtype = {'date': np.str_, 'stock': np.str_, 'name':np.str_, 'buy_price': np.float32, 'sell_price': np.float32, 'acc':np.str_, 'id':np.str_, 'sell_order_price': np.float32, 'sell_avg_price': np.float32, 'sell_time':np.str_, 'state':np.str_, 'amt':np.float32}
    ...
    df.sort_values(by=['date', 'stock'], ascending=[True, True], inplace=True)

    res = []
    for k, g_df in df.groupby(['sd']):
        #if(len(g_df) <= 1):
        #    continue
        g_df = g_df.sort_values(by=['amt'], ascending=[False])
        g_df = g_df.loc[~g_df.sd.duplicated()]
        res.append(g_df)
    ...
    total_df = total_df[['date', 'stock', 'buy_price', 'sell_price', 'open', 'low', 'close', 'ordpoint', 'buy_rate', 'ord_cnt', 'buy_cnt', 'sell_order_price', 'sell_avg_price', 'sell_time', 'state', 'open_close', 'amt']]
    print('############统计##############')
    total_df.sort_values(by=['date', 'stock'], ascending=[True, True], inplace=True)
    print(total_df)
