...
class StockData_day_neu_mse(data.Dataset):
    ...
    def get_batch_data(self, batch_size=-1):
        if self.idx == 0:
            ...
            y = torch.clip(self.y, -0.2, 0.2)
            y = torch.round(y*100)
            y = torch.clip(y, -20, 20)
            y = (y + 20).to(torch.int64)
            y = F.one_hot(y)
            self.yy = y.to(torch.float32)
...
class StockData_mini_batch_day_inday_tensor(data.Dataset):
    def __init__(self, fn, device = 'cpu'):
        ...
        #self.x_max = self.x.abs().max(dim=0).values
        #self.x_max = torch.where(self.x_max < 1, torch.full_like(self.x_max, 1), torch.full_like(self.x_max, 100))
...
class StockData_mini_batch_tensor_gbm(data.Dataset):
    def __init__(self, fn, file_len=0, device=torch.device('cpu')):
        ...
        fp = open(fn, 'rb')
        d = pickle.load(fp)
        self.x = d['x'].to(device).float()
        self.y = d['y'].to(device).float()
        ...
        #tmp_df['adj_return'] = tmp_df['return'] - tmp_df['dmean']
        #print(tmp_df)
        #self.y = torch.tensor(tmp_df.adj_return.to_numpy()).float()

        self.y = torch.log(self.y + 1.0)

        #self.y = torch.where(torch.logical_and(self.y > -0.005, self.y < 0.005), torch.full_like(self.y, 0.0), self.y)
...
class StockData_mini_batch_tensor_gbm_pre(data.Dataset):
    def __init__(self, fn, file_len=0, device=torch.device('cpu')):
        ...
        self.y = torch.log(self.y + 1.0).float()
        #self.y = torch.where(torch.logical_and(self.y > -0.005, self.y < 0.005), torch.full_like(self.y, 0.0), self.y)
...
class StockData_torch_fea_test(data.Dataset):
    def __init__(self, fn, device = 'cpu'):
