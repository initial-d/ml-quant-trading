...
model_saving_path = '/da2/search/wanghexiang/stock_data/save/model_v1.2_new.pt'
forced_saving_path = '/da2/search/wanghexiang/stock_data/save/model_force_v1.2_new.pt'
device = torch.device('cuda:3')

dir_path = '/da2/search/wanghexiang/stock_data/split'

global_lr = 0.01
model = StockPred_v1().to(device)
...
loss_stat = []
best_loss = 100000
dataset = StockData('sample_train_data.txt', 142044295)
...
for epoch in np.arange(n_epochs):
    ...
    for batch_num in range(total_batches):
        ...
        if batch_num != 0 and batch_num % test_model_counter == 0:
            ...
            pre_list = []
            label_list = []
            _valid_data = StockData_trend('test_data', 6296996)
            valid_batch = 256
