...
dir_path = '/home/duyimin/trade_predict'
...
if 1:       
    ...
    for i in range(total_valid):
        t_test_data, l_test_data = _valid_data.get_batch_data(valid_batch)
        label_list = (l_test_data.numpy().reshape(-1)).tolist()
        model.eval()
        with torch.no_grad():
            t_test_data = torch.tensor(t_test_data, dtype=torch.float64)
