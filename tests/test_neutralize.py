...
def test_neutralize_industry_perfectly_explained():
    ...
    The signal is exactly ``X @ beta`` for a valid one-hot industry design.
    After neutralisation the residual should be near machine precision, not
    amplified unit-variance noise.
    """
    T, N = 10, 20
    X = _make_onehot(N, 4)
    beta = torch.tensor([1.5, -2.0, 0.7, 0.2])
...
def test_neutralize_industry_preserves_small_signal_scale():
    """A small but non-degenerate signal is normalized rather than zeroed."""
    T, N = 4, 20
    x = torch.arange(N, dtype=torch.float32).mul(1e-8).expand(T, -1)
    mask = torch.ones(T, N, dtype=torch.bool)
    intercept = torch.ones(T, N, 1)

    out = neutralize_industry(x, mask, intercept)

    assert torch.allclose(
        out.std(dim=1, unbiased=False),
        torch.ones(T),
        atol=1e-5,
    )
