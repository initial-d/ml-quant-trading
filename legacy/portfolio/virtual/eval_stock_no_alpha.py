...
def evaluate(fn):
    ...
    names = ['date', 'stock', 'pred', 'return', 'weight', 'expret', 'std']
    dtype = {'date': np.str_, 'stock': np.str_, 'return': np.float32,  'pred': np.float32, 'weight': np.float32, 'expret': np.float32, 'std':np.float32}
    ...
    #df['pred'] = df['pred'] ** 2.6
    #df = df.dropna()
    df = df.loc[df['weight'] >= 0.001]
    #print(df)
    ret = 1
    for k, g_df in df.groupby(by='date'):
        w = g_df['pred'].mean()
        x = 1
        #g_df = g_df.loc[g_df['weight'] >= 0.01]
        d = g_df.head(MAX_DAILY_TRADE_CNT)
        rets = (d['return'] * d['weight'] / d['weight'].sum()).sum()
