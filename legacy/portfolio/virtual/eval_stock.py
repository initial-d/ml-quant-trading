...
def evaluate(fn):
    ...
    dtype = {'date': np.str_, 'stock': np.str_, 'return': np.float32,  'pred': np.float32, 'weight': np.float32, 'expret': np.float32, 'std':np.float32, 'alpha': np.float32}
