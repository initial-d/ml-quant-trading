...
def generate_trade_df():
    ...
    res = []
    for key, g_df in daily_df.groupby('stock'):
        g_df['fhzs_1'] = g_df['fhzs'].shift(-1)
        g_df['after_open'] = g_df['open'].shift(-1)
        g_df['after_high'] = g_df['high'].shift(-1)
        g_df['after_close'] = g_df['close'].shift(-1)
        g_df['last_volume'] = g_df['volume'].shift(1)
        g_df['last_avg'] = g_df['avg'].shift(1)
        res.append(g_df)
    ...
    #daily_df['after_high'] = daily_df['high'].shift(-1)
    #daily_df['after_close'] = daily_df['close'].shift(-1)
    ...
    #df = df.loc[  ((df.limit_up / df.halfday_close - 1) <= 0.005) & (df.last_volume * df.last_avg * 100 > 50000000) ] 
    #df = df.loc[ ((df.limit_up / df.halfday_close - 1) > 0.01) & (df.last_volume * df.last_avg * 100 > 50000000) ] 
    ...
    df['fix_return'] = (df['after_close'] - df['halfday_close']) / df['halfday_close']
    #df['fix_return'] = (df['after_open'] - df['halfday_close']) / df['halfday_close']
    #df['after_return'] = np.where(df['after_high'] > df['today_close'], (df['today_close'] - df['halfday_close']) / df['halfday_close'], (df['after_close'] - df['halfday_close']) / df['halfday_close'])
    ...
    #df = df.loc[(df['open'] > 10)] 
    #df['fix_return'] = np.where(df['after_high'] > df['halfday_close'], (df['halfday_close'] - df['halfday_close']) / df['halfday_close'], df['fix_return'])
    df['fix_return'] = np.where(df['after_open'] < df['halfday_close'], df['fix_return'], (df['after_open'] - df['halfday_close']) / df['halfday_close'])
    ...
    #df.sort_values(by=['date', 'hpred'], ascending=[True, True], inplace=True)
    #df.to_csv('trade.txt', sep=' ', header=True, index=False, columns=['date', 'stock', 'hpred', 'fix_return', 'fix_return_2'])
