...
model_saving_path = './lr0001_v7/model.pt'
forced_saving_path = './lr0001_v7/model_force.pt'
#device = torch.device('cuda:0')
device = torch.device('cpu')


global_lr = 0.01
model = StockPred_v7().to(device)
...
for epoch in np.arange(n_epochs):
    ...
    for k, v in model.named_parameters():
        ...
        if k == ('layer_' + str(id) + '_1.W.weight'):
            v.requires_grad=True
        if k == ('layer_' + str(id) + '_1.W.bias'):
            v.requires_grad=True
            
        if k == ('layer_' + str(id) + '_2.W.weight'):
            v.requires_grad=True
        if k == ('layer_' + str(id) + '_2.W.bias'):
            v.requires_grad=True

        if k == ('layer_' + str(id) + '_3.W.weight'):
            v.requires_grad=True
        if k == ('layer_' + str(id) + '_3.W.bias'):
            v.requires_grad=True
    ...
    for batch_num in range(total_batches):
        ...
        t_datas = torch.split(t_data, 189, dim=1)
        ...
        optimizer.zero_grad()
        pre = model.forward(t_datas[0].to(device), t_datas[1].to(device), id)
    ...
    if epoch > 0 and epoch % 14 == 0: 
        ...
        for i in range(15):
            ...
            fo = open("./lr0001_v7/tushare_day." + str(epoch // 15) + '.' + str(i), "w")
            for j in range(total_valid):
                ...
                model.eval()
                with torch.no_grad():
                    t_test_datas = torch.split(t_test_data, 189, dim=1)
                    pre = model.valid_batch(t_test_datas[0].to(device), t_test_datas[1].to(device), i)
        ...
        torch.save(model.state_dict(), forced_saving_path + '.' + str(epoch // 15))
